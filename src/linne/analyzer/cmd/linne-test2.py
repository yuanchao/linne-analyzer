...
classifier = joblib.load("linne2.pkl")

sk = []
sk0 = []
sk1 = []
skn = [[] for x in xrange(6)]
row = []
...
for idx in range(0, len(bins)) :
  ...
  for ii in range(1, 7) :
    skn[ii-1].append(classifier.predict_proba(row)[:,ii]*5)
...
for ii in range(5, -1, -1) :
  plot(bins, skn[ii], label=files[ii].split('/')[-1].split('.wav')[0])
