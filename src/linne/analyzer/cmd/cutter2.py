...
#            print "Fall cut ", varName, " at time, value: ", self._sampling[self._index]["Timestamp"], self._sampling[self._index][varName]
    ...
    def find_vowel_id(self, target, duration = 0.):
        ...
        try:
            ...
            if vow_start == currIdx+1:  # first vowel
                vow_start = int(vow_start + 0.05 * vow_len)
            else:
                vow_start = int(vow_start + 0.05 * vow_len)
    ...
    def skip_tail(self):
        try:
            endTime = len(self._sampling) - 1
            self._index = len(self._sampling) - 1

            while self._sampling[self._index]["Spectrum Variance"] < self._maxSV/100 and self._index > 0:
    ...
    def process(self,soundTable,sampling,target):
        ...
        try:
            ...
            if target.lower().find('h') ==0 or \
               ...
               target.lower().find('k') ==0 or \
               target.lower().find('zw') ==0 or \
